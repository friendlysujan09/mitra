...
def home(request):
    ...
    for user in all_profile:
        if user not in my_user:
            ...
            my_age = int(user_profile.age)
            lower_limit = my_age-10
            upper_limit = my_age+10

            if lower_limit <= user_age <= upper_limit:
                suggestion_users.append(user)
...
def search(request):
    ...
    if request.method == 'POST':
        search_command = request.POST.get('username', '')
        if search_command:
            # Simplified query to retrieve profiles based on username
            username_profile_list = Profile.objects.filter(username__icontains=search_command)
            post_list = Post.objects.filter(title_tag__icontains= search_command)

    return render(request, 'base/search.html', {'post_list':post_list, 'user_profile': user_profile,
                                                'username_profile_list':
        username_profile_list})
